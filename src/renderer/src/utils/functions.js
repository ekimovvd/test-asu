...
export async function downloadTest1({ data, organizationName, asuName, result }) {
  ...
  if (data.length) {
    const lines2 = doc.splitTextToSize("        " + `Для повышения уровня защищенности автоматизированной системы управления ${asuName} вам рекомендуется реализовать меры указанные в таблице ниже.`, textWidth);

    doc.text(lines2, 10, 80);
